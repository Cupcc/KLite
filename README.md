...
**作者简介：** 蒋晓岗，男，处女座，2013年毕业于成都信息工程大学计算机科学与技术专业，从事嵌入式软件开发工作近十年，追求精简与干净整洁的编码风格。
...
+ uint32_t kernel_version(void);  
	...
	描述：此函数可以获取KLite的版本号
	> 命名规则：
	主版本：架构调整，主版本变更可能会与旧版本出现不兼容
	次版本：功能调整，次版本变更可能涉及新增或删除功能
	修订号：细节优化或BUG修复为主。
________________________________________________________________________________
+ uint32_t kernel_time(void);  
	...
	> 此函数没有命名为`kernel_get_time()`，一方面是缩短函数名看起来更整齐，
	另外因为time和上面的version一样只有读操作，没有写操作，不区分get和set

________________________________________________________________________________
+ void kernel_idle(void);  
	参数：无  
	返回：无  
	描述：处理内核空闲事务，回收线程资源此函数不会返回。必须单独创建一个线程，并在线程内直接调用。
...
+ uint32_t kernel_idle_time(void);  
	参数：无  
	返回：系统空闲时间(毫秒)  
	描述：获取系统从启动到现在空闲线程占用CPU的总时间，可使用此函数和`kernel_time()`一起计算CPU占用率
